...
**Last updated:** Mon Sep 12 09:54:03 CEST 2022  
...
You can then execute your native executable with: `./build/code-with-quarkus-1.0.16-SNAPSHOT-runner`
...
```bash
docker run --rm -p 8080:8080 --name="quarkus-jpa-example-1.0.16"  \
-it ../releases/de/ochmanski/microservices/quarkus-jpa-example:1.0.16
```
...
```bash
cd $AWS/de/ochmanski/microservices/quarkus-jpa-example
docker run -it --rm -p 8080:8080 --name="quarkus-jpa-example-1.0.16" \
--mount type=bind,source=$PWD/src/main/resources/application-docker.yml,\
target=/work/config/application.yml \
-e QUARKUS_DATASOURCE_PASSWORD=$QUARKUS_DATASOURCE_PASSWORD \
releases/de/ochmanski/microservices/quarkus-jpa-example:1.0.16
```
...
`1.0.16-SNAPSHOT`, you may run it in the following manner:

```bash
cd $AWS/de/ochmanski/microservices/quarkus-jpa-example
docker run --rm -p 8080:8080 --name="quarkus-jpa-example-1.0.16-SNAPSHOT" \
--mount type=bind,source=$PWD/src/main/resources/application-docker.yml,\
target=/work/config/application.yml \
-it snapshots/de/ochmanski/microservices/quarkus-jpa-example:1.0.16-SNAPSHOT
```
...
look at the output of the gradle command in the console. Assuming that the created docker image is identified as:
`1.0.16-SNAPSHOT`, you may open it in the following manner:

```bash
cd $AWS/de/ochmanski/microservices/quarkus-jpa-example
docker run --rm -ti --privileged --entrypoint /bin/sh --rm -p 8080:8080 --name="quarkus-jpa-example-1.0.16-SNAPSHOT" \
--mount type=bind,source=$PWD/src/main/resources/application-docker.yml,\
target=/work/config/application.yml \
-it snapshots/de/ochmanski/microservices/quarkus-jpa-example:1.0.16-SNAPSHOT
```
